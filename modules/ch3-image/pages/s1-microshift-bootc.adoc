...
Deploy MicroShift on image mode for RHEL by building bootc container images and custom installation ISOs.
...
As a reminder, the following figure depicts the overall workflow for building bootc container images and deploying them on edge devices, using image mode for RHEL.
...
What the image mode for RHEL documentation call *physically bound containers* are just container image layers that are stored, as regular files, in a bootc container image.
