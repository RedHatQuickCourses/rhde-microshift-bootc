...
Provision an image mode system with MicroShift, fully disconnected, from an installation ISO.
...
You succeed in provisioning an edge device, from an installable ISO, in an air-gapped environment: you only need the installation media to fully provision a ready-to-use image mode system running a MicroShift instance.
